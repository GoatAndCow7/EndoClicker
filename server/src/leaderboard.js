...
leaderboardRouter.get('/leaderboard', (req, res) => {
  const t = now();
  // Score = total à vie (jamais remis à zéro) : renaître ne fait pas
  // chuter au classement. On récupère plus que 20 : les états
  // impossibles sont filtrés ensuite.
  const rows = db
    .prepare(
      `SELECT u.pseudo, u.created_at, s.total_endocraft, s.lifetime_endocraft, s.achievements, s.renaissances, s.state, s.updated_at
       FROM states s JOIN users u ON u.id = s.user_id
       ORDER BY s.lifetime_endocraft DESC
       LIMIT 60`
    )
    .all();
  ...
  res.json({
    leaderboard: clean.slice(0, 20).map((r) => {
      ...
      return {
        pseudo: r.pseudo,
        totalEndocraft: r.lifetime_endocraft || r.total_endocraft,
...
leaderboardRouter.get('/profile/:pseudo', (req, res) => {
  ...
  const row = db
    .prepare(
      'SELECT state, total_endocraft, lifetime_endocraft, updated_at FROM states WHERE user_id = ?'
    )
    .get(user.id);

  const state = row ? JSON.parse(row.state) : {};
  const lifetime = row ? row.lifetime_endocraft || 0 : 0;
  // Le rang suit le total à vie, comme le classement
  const rank = row
    ? db
        .prepare(
          'SELECT COUNT(*) + 1 AS r FROM states WHERE lifetime_endocraft > ?'
        )
        .get(lifetime).r
    : null;
